...
internal static class RequestTests
{
    public static async Task Run(ProjectRuntime runtime)
    {
        ...
        var admin = new ProjectAdminService(token);
        ...
        async Task Check(string path, string method, int expected, Action<HttpContext>? configure = null)
        {
            ...
            var called = false;
            await new ProjectRequestMiddleware(_ => { called = true; return Task.CompletedTask; }).InvokeAsync(context, runtime, admin);
        ...
        await Check("/api/projects", "GET", 401);
        await Check("/api/projects/status", "GET", 200);
        await Check("/api/projects", "GET", 200, c => c.Request.Headers["X-CREC-Admin"] = token);
        await Check("/api/projects", "GET", 401, c => c.Request.Headers["X-CREC-Admin"] = "wrong-token");
        await Check("/api/projects/switch", "POST", 403, c => c.Request.Headers["X-CREC-Admin"] = token);
        await Check("/api/projects/switch", "POST", 200, c => {
            c.Request.Headers["X-CREC-Admin"] = token;
        ...
        await Check("/api/projects/login", "POST", 403);
        await Check("/api/projects/login", "POST", 200, c => {
            c.Request.Headers["X-CREC-Request"] = "1";
        ...
        login.Request.Host = new HostString("localhost", 5000);
        admin.SignIn(login);
        var cookie = login.Response.Headers.SetCookie.ToString();
        if (!cookie.Contains("httponly", StringComparison.OrdinalIgnoreCase) || !cookie.Contains("samesite=strict", StringComparison.OrdinalIgnoreCase))
            throw new Exception("Session cookie protection missing");
        await Check("/api/projects", "GET", 200, c => c.Request.Headers.Cookie = cookie.Split(';')[0]);
    }
}
